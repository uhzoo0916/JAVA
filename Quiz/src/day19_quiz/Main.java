...
public class Main {
		...
		public static void main(String[] args) throws Exception{
			...
			ArrayList<Information> Bmi = new ArrayList<Information>();
			...
			while(true) {	
				...
				case 1 : //1. 신체 정보 입력 
					...
					bmi = (double) weight / (height / 100.0) * (height / 100.0);
						...
						if(bmi<=18.5) {	
							System.out.println("당신의 BMI =" + bmi+ "판정 : 저체중");
						}
						else if((bmi>18.5) &&(bmi<=22.9)) {	
							System.out.println("당신의 BMI = " + bmi + "판정 : 정상");
							
						}
						else if ((bmi>=23.9) &&(bmi<=24.9)) {	
							System.out.println("당신의 BMI = " + bmi + "판정 : 과체중");
						}
						else if (bmi>=25.0) {	
							System.out.println("당신의 BMI = " + bmi+ "판정 : 비만");
						}
						
					 
					
					Bmi.add(new Information(name, height, weight, gender,bmi,result));
				...
				case 5 : //5. 비만도 측정 
							...
							double bmiCheck = weight / (height/100.0) *(height /100.0);
